Feature: Manage translation files for a WordPress install
  ...
  Scenario: Plugin translation CRUD
    ...
    When I run `wp language plugin install en_GB`
    And I run `wp language plugin install en_AU`
    ...
    When I run `wp language plugin install en_CA en_NZ`
    ...
    When I run `ls {SUITE_CACHE_DIR}/translation | grep plugin-default-`
    Then STDOUT should contain:
      """
      en_AU
      """
    ...
    When I try `wp language plugin install en_AU`
    ...
    When I run `wp language plugin activate en_GB`
    Then STDOUT should be:
      """
      Success: Language activated.
      """

    When I run `wp language plugin list --fields=language,english_name,update`
    Then STDOUT should be a table containing rows:
    ...
    When I try `wp language plugin install en_AU --activate`
    ...
    When I try `wp language plugin install en_CA en_NZ --activate`
    Then STDERR should be:
      """
    ...
    When I run `wp language plugin uninstall en_GB`
    ...
    When I run `wp language plugin uninstall en_CA en_NZ`
    ...
    When I run `wp language plugin install en_GB --activate`
    Then the wp-content/languages/admin-en_GB.po file should exist
    And the wp-content/languages/en_GB.po file should exist
    And STDOUT should contain:
      """
    ...
    When I try `wp language plugin install invalid_lang`
    Then STDERR should be:
      """
  ...
  Scenario: Ensure correct language is installed for WP version
    ...
    When I run `wp language plugin install nl_NL`
    Then STDOUT should contain:
      """
